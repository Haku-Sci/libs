...
export class DebugLauncherPanel {
    public static currentPanel: DebugLauncherPanel | undefined;
    ...
    private async _updateWebviewContent() {
        try {
            ...
            const htmlFilePath = config.get<string>('htmlFile', 'src/webview.html');
    ...
    private async _launchDebugConfiguration(configName: string) {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (!workspaceFolder) {
    ...
    private async _sendDebugConfigurations() {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (!workspaceFolder) {
                return;
            ...
            const config = vscode.workspace.getConfiguration('debugLauncher');
            const launchJsonPath = config.get<string>('launchJsonPath', '.vscode/launch.json');
            ...
            try {
                ...
                this._panel.webview.postMessage({
                    ...
                    configurations: configurations.map((config: any) => config.name)
                });
            } catch {
    ...
    private async _sendServices() {
        try {
            ...
            const config = vscode.workspace.getConfiguration('debugLauncher');
            const servicesJsonPath = config.get<string>('servicesJsonPath', 'services.json');
